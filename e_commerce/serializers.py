...
from .models import (
    Categorie, FichierProduit, ImageSlider, Produit, Client, ProduitPanier, 
    ProduitCommande, Panier, Commande, NoteDeRecommandation, Contact, SiteSettings
)    
...
class ProduitSerializer2(serializers.ModelSerializer):
    ...
    class Meta:
        model = Produit
        fields = ("id", "nom_produit", "prix_produit", "categorie", "fichierproduit_set", "nouveau")
...
class ProduitPanierSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProduitPanier
        fields = (
            "id_produit_panier", "id_produit", "nom_produit", "qte_produit_panier",
            "prix_produit", "qte_stock_produit",
            # "image_produit", 
        )
...
class PanierSerializer2(serializers.ModelSerializer):
    total = serializers.DecimalField(read_only=True, max_digits=8, decimal_places=2)
...
class ClientSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            "adresse_livraison", "genre", "adresse", "panier", "tel", 
...
class ProduitCommmandeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProduitCommande
        fields = (
            "id_produit_commande", "id_produit", "nom_produit", 
...
class CommandeSerializer(serializers.ModelSerializer):
    total = serializers.DecimalField(read_only=True, max_digits=8, decimal_places=2)
    client = ClientSerializer()
    produits = ProduitCommmandeSerializer(source='produitcommande_set', many=True)
    class Meta:
        model = Commande
        fields = "__all__"
